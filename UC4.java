...
public class UC4 {
			...
			static int total=0;//Class level variables
			...
			static void rolldie() { 			//  use method  to roll the die

				int min=1;
				int max=6;
				int dieNumber = (int) Math.floor(Math.random()*(max-min)+min); // random function call
			
				System.out.println("Die = "+dieNumber);
			

				if (total <= win)  //CHECK condition
				{
			...
			}			static void checkpay(int check) 
